...
public class Exercise8Test extends CompanyDomainForKata
{
    ...
    {
        MutableSortedBag<Double> orderedPrices = company.getOrders().flatCollect(Order::getLineItems).select(li -> li.getValue() > 7.5).collect(LineItem::getValue).toSortedBag(Comparator.reverseOrder());
    ...
    {
        MutableList<Customer> customersWithSaucers = company.getCustomers().select(c -> c.getOrders().flatCollect(Order::getLineItems).collect(LineItem::getName).contains("saucer"));
    ...
    {
        MutableListMultimap<Double, Customer> multimap = company.getCustomers().groupBy(c -> c.getOrders().flatCollect(Order::getLineItems).collect(LineItem::getValue).max());
